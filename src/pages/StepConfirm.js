...
export default function StepConfirm({ service, slot, client }) {
  ...
  return (
    <div className="fade-up">
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--p500)', textTransform: 'uppercase', letterSpacing: .6, marginBottom: 12 }}>
          Appointment
        </div>
        <SummaryRow label="Service"  value={service.name} />
        <SummaryRow label="Date"     value={format(dt, 'EEEE, d MMMM yyyy')} />
        <SummaryRow label="Time"     value={format(dt, 'HH:mm')} />
        <SummaryRow label="Duration" value={`${service.duration_mins} min`} />
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0 0', fontSize: 15 }}>
          <span style={{ fontWeight: 500, color: 'var(--p800)' }}>Total</span>
          <span style={{ fontWeight: 500, color: 'var(--p700)' }}>{formatPrice(service.price)}</span>
        </div>
      </div>
